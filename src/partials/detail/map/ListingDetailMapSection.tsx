...
const ListingDetailMapSection: FC<Props> = ({ coordinates, isStrict }) => {
  ...
  useEffect(() => {
    fetchNearestPlaces(coordinates)
      .then((res) => {
        setPlaces(res)
        setIsError(false)
      })
      ...
      {!loading && isError && (
        <div className='pb-2 pt-4'>
          <Alert showIcon type='error'>
            ...
            <Alert.Description>{t('sections.map.error.description')}</Alert.Description>
          </Alert>
        </div>
      )}
      {!loading && !isError && (
        <ListingDetailPlaceMapSection
          isStrict={isStrict}
