...
const ListingFilterValidationGroup = () => {
  ...
  return (
    <div className='max-h-60 space-y-2 overflow-y-auto md:max-h-full md:overflow-y-hidden'>
      {ValidationKeys.map((key) => (
        <ListingFilterBooleanItem
          key={key}
          info={t(`components.validation.${key}.description`)}
          label={t(`components.validation.${key}.label`)}
          name={key}
          queryKey={key}
        />
      ))}
